...
def add_Assessment():
    ...
    ASSESSMENTS = [Assessment, date_created]

    c.execute('INSERT INTO Assessments (Assessment, date_created) VALUES (?,?)',ASSESSMENTS)
...
def add_Assessment_Result():
    ...
    c.execute('INSERT INTO Assessment_Results (user,assessment_result_name, assessment, score, date_taken, manager)VALUES (?,?,?,?,?,?',ASSESSMENT_RESULTS)
    conn.commit

def edit_Assessment_Result():
    c.execute('UPDATE Assessment_Results SET assessment_results_name = ? WHERE assessment_results_id =?',(asr_value,assessment_results_id))
...
def feedbackasr():
    c.execute('SELECT Assessment_Results,assments_results_name FROM Assessment_Results WHERE assessment_results_id = ?')
    data = c.fetchone()
...
if choice == '1':
    ...
    add_edit_menu()
    response = input('Which Competency?:')
    add_Comptetency(response)
    
    feedbackcomp(response)
    ...
    add_edit_menu()
    response = input('Which Assessment Result?:')
    add_Assessment_Result(response)
    
    feedbackasr(response)

elif choice == '8':
    
    add_edit_menu()
    assessment_results_id = input('Which Assessment Results?:')
    asr_value = input('Please provide a new value for this Assessment Result:')
    edit_Assessment_Result(asr_value,assessment_results_id)
    
    feedbackasr(assessment_results_id)
...
# Execute the query
    cursor.execute('SELECT * FROM Users,Competencies,Assessments,Assessment_Results ')
# Get data in batches
while True:
    # Read the data
